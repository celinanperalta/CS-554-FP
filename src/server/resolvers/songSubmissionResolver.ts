...
export class SongSubmissionResolver {
    ...
    async addSongSubmission(
        ...
        if(!isAuthenticated(ctx)){throw "Must Authenticate to add song sub"}
        return await songSubmissionService.addSongSubmission(prompt_id,song,getUserFromContext(ctx));
    }
    ...
    async updateSongSubmission(
        ...
        if(!isAuthenticated(ctx) || getUserFromContext(ctx)!==submitted_by){throw "Error: must authenticate/can't update another users song sub"}
    ...
    @Mutation(returns => SongSubmission, {nullable: true})
    async deleteSongSubmission(@Arg("id") id: string, @Ctx() ctx: UserLoginContext,) : Promise<SongSubmission>{
        ...
        if(!isAuthenticated(ctx) && getUserFromContext(ctx)!==songSub.submitted_by && getUserFromContext(ctx)!==prompt.posted_by){
